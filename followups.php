...
/**
 * followups.php
 * Subhiksha Cards ERP - Follow-ups
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_record') {
            if (!mp_table_exists($conn, 'followups')) {
                throw new RuntimeException('followups table is missing. Run the SQL file first.');
            }

            $id = mp_int($_POST['id'] ?? 0);
            $customer_name = mp_post('customer_name');
            $mobile = mp_post('mobile');
            $followup_date = mp_post('followup_date');
            $remarks = mp_post('remarks');
            $status = mp_post('status', 'Active');

            if ($customer_name === '' || $mobile === '' || $followup_date === '') {
            ...
            if ($id > 0) {
                $stmt = $conn->prepare("
                    UPDATE followups
                    SET customer_name=?, mobile=?, followup_date=?, remarks=?, status=?, updated_at=NOW()
                    WHERE id = ?
                ");
                $stmt->bind_param('sssssi', $customer_name, $mobile, $followup_date, $remarks, $status, $id);
                $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare("
                INSERT INTO followups
                    (customer_name, mobile, followup_date, remarks, status, created_by, created_at, updated_at)
                VALUES
                    (?,?,?,?,?, ?, NOW(), NOW())
            ");
            $createdBy = (int)($_SESSION['user_id'] ?? 0);
            $stmt->bind_param('sssssi', $customer_name, $mobile, $followup_date, $remarks, $status, $createdBy);
            $stmt->execute();
            $stmt->close();
        ...
        if ($action === 'delete_record') {
            ...
            $stmt = $conn->prepare("UPDATE followups SET status='Inactive', updated_at=NOW() WHERE id=?");
...
$msg = (string)($_GET['msg'] ?? '');
if ($msg === 'created') $message = 'Follow-ups created successfully.';
elseif ($msg === 'updated') $message = 'Follow-ups updated successfully.';
elseif ($msg === 'deleted') $message = 'Follow-ups disabled successfully.';

$rows = [];
if (mp_table_exists($conn, 'followups')) {
    try {
        $res = $conn->query("SELECT * FROM followups ORDER BY id DESC LIMIT 300");
                ...
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Manage follow-ups records.</p>
                        </div>
                ...
                <div class="card-ui module-card">
                    ...
                    <div class="table-responsive desktop-table">
                        <table class="table-ui" id="dataTable">
                            <thead>
                                <tr>
                                    <th>Customer Name</th><th>Mobile</th><th>Followup Date</th><th>Remarks</th><th>Status</th>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($rows as $row): ?>
                                    <tr>
                                        <td><?= e((string)($row['customer_name'] ?? '-')) ?></td><td><?= e((string)($row['mobile'] ?? '-')) ?></td><td><?= e((string)($row['followup_date'] ?? '-')) ?></td><td><?= e((string)($row['remarks'] ?? '-')) ?></td><td><span class="status-pill <?= strtolower((string)$row['status']) === 'active' ? 'active' : 'pending' ?>"><?= e($row['status'] ?? 'Pending') ?></span></td>
                                        <td class="text-end">
                                            <button type="button"
...
data-followup-date="<?= e($row['followup_date'] ?? '') ?>"
                        ...
                        <?php foreach ($rows as $row): ?>
                            <div class="mobile-card">
                                <div class="d-flex justify-content-between gap-2">
                                    <div>
                                        <div class="mobile-card-title"><?= e((string)($row['customer_name'] ?? 'Follow-ups')) ?></div>
...
<span class="mobile-card-subtitle">Follow-up Date: <?= e((string)($row['followup_date'] ?? '-')) ?></span>
<span class="mobile-card-subtitle">Remarks: <?= e((string)($row['remarks'] ?? '-')) ?></span>
                                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form method="post" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="recordModalTitle">Create Follow-ups</h5>
...
<div class="col-md-6"><label class="form-label fw-bold">Follow-up Date</label><input type="date" name="followup_date" id="followup_date" class="form-control"></div>
<div class="col-12"><label class="form-label fw-bold">Remarks</label><textarea name="remarks" id="remarks" rows="3" class="form-control"></textarea></div>
    ...
    <script>
        (function () {
            ...
            document.getElementById('newRecordBtn')?.addEventListener('click', function () {
                title.textContent = 'Create Follow-ups';
