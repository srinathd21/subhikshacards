...
/**
 * quotations.php
 * Subhiksha Cards ERP - Quotations
 */
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_record') {
            if (!mp_table_exists($conn, 'quotations')) {
                throw new RuntimeException('quotations table is missing. Run the SQL file first.');
            }

            $id = mp_int($_POST['id'] ?? 0);
            $quotation_no = mp_post('quotation_no');
            $customer_name = mp_post('customer_name');
            $mobile = mp_post('mobile');
            $total_amount = mp_post('total_amount');
            $notes = mp_post('notes');
            $status = mp_post('status', 'Active');

            if ($quotation_no === '' || $customer_name === '' || $mobile === '' || $total_amount === '') {
            ...
            if ($id > 0) {
                $stmt = $conn->prepare("
                    UPDATE quotations
                    SET quotation_no=?, customer_name=?, mobile=?, total_amount=?, notes=?, status=?, updated_at=NOW()
                    WHERE id = ?
                ");
                $stmt->bind_param('sssdssi', $quotation_no, $customer_name, $mobile, $total_amount, $notes, $status, $id);
                $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare("
                INSERT INTO quotations
                    (quotation_no, customer_name, mobile, total_amount, notes, status, created_by, created_at, updated_at)
                VALUES
                    (?,?,?,?,?,?, ?, NOW(), NOW())
            ");
            $createdBy = (int)($_SESSION['user_id'] ?? 0);
            $stmt->bind_param('sssdssi', $quotation_no, $customer_name, $mobile, $total_amount, $notes, $status, $createdBy);
            $stmt->execute();
            $stmt->close();
        ...
        if ($action === 'delete_record') {
            ...
            $stmt = $conn->prepare("UPDATE quotations SET status='Inactive', updated_at=NOW() WHERE id=?");
...
$msg = (string)($_GET['msg'] ?? '');
if ($msg === 'created') $message = 'Quotations created successfully.';
elseif ($msg === 'updated') $message = 'Quotations updated successfully.';
elseif ($msg === 'deleted') $message = 'Quotations disabled successfully.';

$rows = [];
if (mp_table_exists($conn, 'quotations')) {
    try {
        $res = $conn->query("SELECT * FROM quotations ORDER BY id DESC LIMIT 300");
                ...
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Manage quotations records.</p>
                        </div>
                ...
                <div class="card-ui module-card">
                    ...
                    <div class="table-responsive desktop-table">
                        <table class="table-ui" id="dataTable">
                            <thead>
                                <tr>
                                    <th>Quotation No</th><th>Customer Name</th><th>Mobile</th><th>Total Amount</th><th>Status</th>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($rows as $row): ?>
                                    <tr>
                                        <td><?= e((string)($row['quotation_no'] ?? '-')) ?></td><td><?= e((string)($row['customer_name'] ?? '-')) ?></td><td><?= e((string)($row['mobile'] ?? '-')) ?></td><td><?= e((string)($row['total_amount'] ?? '-')) ?></td><td><span class="status-pill <?= strtolower((string)$row['status']) === 'active' ? 'active' : 'pending' ?>"><?= e($row['status'] ?? 'Pending') ?></span></td>
                                        <td class="text-end">
                                            <button type="button"
...
data-total-amount="<?= e($row['total_amount'] ?? '') ?>"
                        ...
                        <?php foreach ($rows as $row): ?>
                            <div class="mobile-card">
                                <div class="d-flex justify-content-between gap-2">
                                    <div>
                                        <div class="mobile-card-title"><?= e((string)($row['quotation_no'] ?? 'Quotations')) ?></div>
                                        <span class="mobile-card-subtitle">Quotation No: <?= e((string)($row['quotation_no'] ?? '-')) ?></span>
...
<span class="mobile-card-subtitle">Total Amount: <?= e((string)($row['total_amount'] ?? '-')) ?></span>
                                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form method="post" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="recordModalTitle">Create Quotations</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label fw-bold">Quotation No</label><input type="text" name="quotation_no" id="quotation_no" class="form-control"></div>
...
<div class="col-md-6"><label class="form-label fw-bold">Total Amount</label><input type="number" name="total_amount" id="total_amount" class="form-control"></div>
    ...
    <script>
        (function () {
            ...
            document.getElementById('newRecordBtn')?.addEventListener('click', function () {
                title.textContent = 'Create Quotations';
            ...
            document.querySelectorAll('.js-edit-record').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    title.textContent = 'Edit Quotations';
                    submit.textContent = 'Update';

                    set('id', btn.dataset.id || '');
                    set('quotation_no', btn.dataset.quotationNo || '');
                    set('customer_name', btn.dataset.customerName || '');
                    set('mobile', btn.dataset.mobile || '');
                    set('total_amount', btn.dataset.totalAmount || '');
