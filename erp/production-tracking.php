...
<body class="<?= e(($theme['layout_density'] ?? '') === 'compact' ? 'layout-compact' : '') ?>">
            ...
            <section class="page-section module-page">
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Track design, approval, printing, cutting, quality and
                                dispatch stages.</p>
                        </div>
                ...
                <div class="card-ui module-card">
                    <div
                        ...
                        <div>
                            <h2 class="module-title"><?= e($stage !== '' ? $stages[$stage] : 'All Production Jobs') ?>
                            </h2>
                    ...
                    <div class="table-responsive desktop-table">
                        <table class="table-ui" id="dataTable">
                            ...
                            <tbody>
                                <?php if (!$rows): ?>
                                <tr>
                                    <td colspan="7" class="text-center text-muted-custom py-4">No jobs found.</td>
                                </tr>
                                <?php endif; ?>

                                <?php foreach ($rows as $row): ?>
                                <tr>
                                    <td><strong><?= e($row['job_no']) ?></strong><span
                                            class="small-muted"><?= e($row['order_no'] ?? '') ?></span></td>
                                    <td><?= e($row['customer_name']) ?><span
                                            class="small-muted"><?= e($row['mobile'] ?? '') ?></span></td>
                                    <td><?= e($row['item_name']) ?><span class="small-muted">Qty:
                                            <?= e($row['quantity'] ?? 0) ?></span></td>
                                    <td><?= e($stages[$row['current_stage']] ?? $row['current_stage']) ?></td>
                                    <td><?= e($row['delivery_date'] ?? '-') ?></td>
                                    <td><span
                                            class="status-pill <?= strtolower((string)$row['status']) === 'completed' ? 'completed' : (strtolower((string)$row['status']) === 'inactive' ? 'inactive' : 'active') ?>"><?= e($row['status'] ?? 'Active') ?></span>
                                    </td>
                                    <td class="text-end">
                                        <button type="button"
                                            class="btn btn-sm btn-outline-primary rounded-pill fw-bold js-edit-job"
                                            data-bs-toggle="modal" data-bs-target="#jobModal"
                                            data-id="<?= e($row['id']) ?>" data-job-no="<?= e($row['job_no']) ?>"
                                            ...
                                            data-notes="<?= e($row['notes']) ?>" data-status="<?= e($row['status']) ?>">
                                            Edit
                                        </button>

                                        <?php if (strtolower((string)($row['status'] ?? 'active')) !== 'inactive'): ?>
                                        <form method="post" class="d-inline"
                                            onsubmit="return confirm('Disable this job?')">
                            ...
                            <div class="d-flex justify-content-between gap-2">
                                <div>
                                    ...
                                    <span class="mobile-card-subtitle"><?= e($row['customer_name']) ?> ·
                                        <?= e($row['mobile']) ?></span>
                                    <span class="mobile-card-subtitle"><?= e($row['item_name']) ?> · Qty
                                        <?= e($row['quantity']) ?></span>
                                    <span class="mobile-card-subtitle">Stage:
                                        <?= e($stages[$row['current_stage']] ?? $row['current_stage']) ?></span>
                                    <span class="mobile-card-subtitle">Delivery:
                                        <?= e($row['delivery_date'] ?? '-') ?></span>
                                </div>
                                <span
                                    class="status-pill <?= strtolower((string)$row['status']) === 'completed' ? 'completed' : 'active' ?>"><?= e($row['status'] ?? 'Active') ?></span>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form method="post" class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label fw-bold">Job No</label><input name="job_no"
                                id="job_no" class="form-control" placeholder="Auto if empty"></div>
                        <div class="col-md-6"><label class="form-label fw-bold">Order No</label><input name="order_no"
                                id="order_no" class="form-control"></div>
                        <div class="col-md-6"><label class="form-label fw-bold">Customer Name *</label><input
                        ...
                        <div class="col-md-6"><label class="form-label fw-bold">Item Name *</label><input
                                name="item_name" id="item_name" class="form-control" required></div>
                        <div class="col-md-3"><label class="form-label fw-bold">Quantity</label><input type="number"
                                name="quantity" id="quantity" class="form-control" value="0"></div>
                        <div class="col-md-3"><label class="form-label fw-bold">Delivery Date</label><input type="date"
                                name="delivery_date" id="delivery_date" class="form-control"></div>
                        <div class="col-md-4"><label class="form-label fw-bold">Stage</label><select
                                name="current_stage" id="current_stage"
                                class="form-select"><?php foreach ($stages as $key => $label): if ($key === '') continue; ?>
                                <option value="<?= e($key) ?>"><?= e($label) ?></option><?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-4"><label class="form-label fw-bold">Priority</label><select name="priority"
                                id="priority" class="form-select">
                                <option>Low</option>
                                <option selected>Normal</option>
                                <option>High</option>
                                <option>Urgent</option>
                            </select></div>
                        <div class="col-md-4"><label class="form-label fw-bold">Status</label><select name="status"
                        ...
                        <div class="col-12"><label class="form-label fw-bold">Notes</label><textarea name="notes"
                                id="notes" rows="3" class="form-control"></textarea></div>
                    </div>
                </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-4 fw-bold" id="jobSubmitBtn">Save
                        Job</button>
                </div>
            </form>
        </div>
    ...
    (function() {
        ...
        document.getElementById('newJobBtn')?.addEventListener('click', function() {
            ...
            ['id', 'job_no', 'customer_name', 'mobile', 'order_no', 'item_name', 'delivery_date', 'notes']
            .forEach(id => set(id, ''));
            set('quantity', 0);
