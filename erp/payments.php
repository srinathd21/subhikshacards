...
/**
 * payments.php
 * Subhiksha Cards ERP - Payments
 */

require_once __DIR__ . '/includes/auth.php';
require_permission($conn, 'can_view', 'payments.php');
...
{
    header('Location: payments.php' . ($query !== '' ? '?' . $query : ''));
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_record') {
            if (!mp_table_exists($conn, 'payments')) {
                throw new RuntimeException('payments table is missing. Run the SQL file first.');
            }

            $id = mp_int($_POST['id'] ?? 0);
            $receipt_no = mp_post('receipt_no');
            $customer_name = mp_post('customer_name');
            $mobile = mp_post('mobile');
            $amount = mp_post('amount');
            $payment_mode = mp_post('payment_mode');
            $notes = mp_post('notes');
            $status = mp_post('status', 'Active');

            if ($receipt_no === '' || $customer_name === '' || $mobile === '' || $amount === '' || $payment_mode === '') {
            ...
            if ($id > 0) {
                $stmt = $conn->prepare("
                    UPDATE payments
                    SET receipt_no=?, customer_name=?, mobile=?, amount=?, payment_mode=?, notes=?, status=?, updated_at=NOW()
                    WHERE id = ?
                ");
                $stmt->bind_param('sssdsssi', $receipt_no, $customer_name, $mobile, $amount, $payment_mode, $notes, $status, $id);
                $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare("
                INSERT INTO payments
                    (receipt_no, customer_name, mobile, amount, payment_mode, notes, status, created_by, created_at, updated_at)
                VALUES
                    (?,?,?,?,?,?,?, ?, NOW(), NOW())
            ");
            $createdBy = (int)($_SESSION['user_id'] ?? 0);
            $stmt->bind_param('sssdsssi', $receipt_no, $customer_name, $mobile, $amount, $payment_mode, $notes, $status, $createdBy);
            $stmt->execute();
            $stmt->close();
        ...
        if ($action === 'delete_record') {
            ...
            $stmt = $conn->prepare("UPDATE payments SET status='Inactive', updated_at=NOW() WHERE id=?");
...
$msg = (string)($_GET['msg'] ?? '');
if ($msg === 'created') $message = 'Payments created successfully.';
elseif ($msg === 'updated') $message = 'Payments updated successfully.';
elseif ($msg === 'deleted') $message = 'Payments disabled successfully.';

$rows = [];
if (mp_table_exists($conn, 'payments')) {
    try {
        $res = $conn->query("SELECT * FROM payments ORDER BY id DESC LIMIT 300");
...
<head>
    ...
    <title>Payments - Subhiksha Cards</title>
            ...
            <section class="page-section module-page">
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Manage payments records.</p>
                        </div>
                ...
                <div class="card-ui module-card">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3 mb-3">
                        <div>
                            <h2 class="module-title">Payments List</h2>
                    ...
                    <div class="table-responsive desktop-table">
                        <table class="table-ui" id="dataTable">
                            <thead>
                                <tr>
                                    <th>Receipt No</th><th>Customer Name</th><th>Mobile</th><th>Amount</th><th>Payment Mode</th><th>Status</th>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($rows as $row): ?>
                                    <tr>
                                        <td><?= e((string)($row['receipt_no'] ?? '-')) ?></td><td><?= e((string)($row['customer_name'] ?? '-')) ?></td><td><?= e((string)($row['mobile'] ?? '-')) ?></td><td><?= e((string)($row['amount'] ?? '-')) ?></td><td><?= e((string)($row['payment_mode'] ?? '-')) ?></td><td><span class="status-pill <?= strtolower((string)$row['status']) === 'active' ? 'active' : 'pending' ?>"><?= e($row['status'] ?? 'Pending') ?></span></td>
                                        <td class="text-end">
                                            <button type="button"
                        ...
                        <?php foreach ($rows as $row): ?>
                            <div class="mobile-card">
                                <div class="d-flex justify-content-between gap-2">
                                    <div>
                                        <div class="mobile-card-title"><?= e((string)($row['receipt_no'] ?? 'Payments')) ?></div>
                                        <span class="mobile-card-subtitle">Receipt No: <?= e((string)($row['receipt_no'] ?? '-')) ?></span>
...
<span class="mobile-card-subtitle">Amount: <?= e((string)($row['amount'] ?? '-')) ?></span>
                                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form method="post" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="recordModalTitle">Create Payments</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label fw-bold">Receipt No</label><input type="text" name="receipt_no" id="receipt_no" class="form-control"></div>
...
<div class="col-md-6"><label class="form-label fw-bold">Amount</label><input type="number" name="amount" id="amount" class="form-control"></div>
<div class="col-md-6"><label class="form-label fw-bold">Payment Mode</label><input type="text" name="payment_mode" id="payment_mode" class="form-control"></div>
    ...
    <script>
        (function () {
            ...
            document.getElementById('newRecordBtn')?.addEventListener('click', function () {
                title.textContent = 'Create Payments';
            ...
            document.querySelectorAll('.js-edit-record').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    title.textContent = 'Edit Payments';
                    submit.textContent = 'Update';

                    set('id', btn.dataset.id || '');
                    set('receipt_no', btn.dataset.receiptNo || '');
                    set('customer_name', btn.dataset.customerName || '');
                    set('mobile', btn.dataset.mobile || '');
                    set('amount', btn.dataset.amount || '');
                    set('payment_mode', btn.dataset.paymentMode || '');
