...
function mp_csrf(): void
{
    if (
        ...
        !hash_equals($_SESSION['job_cards_csrf'], (string)$_POST['csrf_token'])
    ) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_record') {
            if (!mp_table_exists($conn, 'job_cards')) {
                throw new RuntimeException('job_cards table is missing. Run the SQL file first.');
            }

            $id = mp_int($_POST['id'] ?? 0);
            $job_no = mp_post('job_no');
            $customer_name = mp_post('customer_name');
            $mobile = mp_post('mobile');
            $delivery_date = mp_post('delivery_date');
            $job_stage = mp_post('job_stage');
            $notes = mp_post('notes');
            $status = mp_post('status', 'Active');

            if ($job_no === '' || $customer_name === '' || $mobile === '' || $delivery_date === '' || $job_stage === '') {
            ...
            if ($id > 0) {
                $stmt = $conn->prepare("
                    UPDATE job_cards
                    SET job_no=?, customer_name=?, mobile=?, delivery_date=?, job_stage=?, notes=?, status=?, updated_at=NOW()
                    WHERE id = ?
                ");
                $stmt->bind_param('sssssssi', $job_no, $customer_name, $mobile, $delivery_date, $job_stage, $notes, $status, $id);
                $stmt->execute();
                $stmt->close();
            ...
            $stmt = $conn->prepare("
                INSERT INTO job_cards
                    (job_no, customer_name, mobile, delivery_date, job_stage, notes, status, created_by, created_at, updated_at)
                VALUES
                    (?,?,?,?,?,?,?, ?, NOW(), NOW())
            ");
            $createdBy = (int)($_SESSION['user_id'] ?? 0);
            $stmt->bind_param('sssssssi', $job_no, $customer_name, $mobile, $delivery_date, $job_stage, $notes, $status, $createdBy);
            $stmt->execute();
            $stmt->close();
        ...
        if ($action === 'delete_record') {
            ...
            $stmt = $conn->prepare("UPDATE job_cards SET status='Inactive', updated_at=NOW() WHERE id=?");
...
$msg = (string)($_GET['msg'] ?? '');
if ($msg === 'created') $message = 'Job Cards created successfully.';
elseif ($msg === 'updated') $message = 'Job Cards updated successfully.';
elseif ($msg === 'deleted') $message = 'Job Cards disabled successfully.';

$rows = [];
if (mp_table_exists($conn, 'job_cards')) {
    try {
        $res = $conn->query("SELECT * FROM job_cards ORDER BY id DESC LIMIT 300");
                ...
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Manage job cards records.</p>
                        </div>
                ...
                <div class="card-ui module-card">
                    ...
                    <div class="table-responsive desktop-table">
                        <table class="table-ui" id="dataTable">
                            <thead>
                                <tr>
                                    <th>Job No</th><th>Customer Name</th><th>Mobile</th><th>Delivery Date</th><th>Job Stage</th><th>Status</th>
                            ...
                            <tbody>
                                ...
                                <?php foreach ($rows as $row): ?>
                                    <tr>
                                        <td><?= e((string)($row['job_no'] ?? '-')) ?></td><td><?= e((string)($row['customer_name'] ?? '-')) ?></td><td><?= e((string)($row['mobile'] ?? '-')) ?></td><td><?= e((string)($row['delivery_date'] ?? '-')) ?></td><td><?= e((string)($row['job_stage'] ?? '-')) ?></td><td><span class="status-pill <?= strtolower((string)$row['status']) === 'active' ? 'active' : 'pending' ?>"><?= e($row['status'] ?? 'Pending') ?></span></td>
                                        <td class="text-end">
                                            <button type="button"
...
data-job-stage="<?= e($row['job_stage'] ?? '') ?>"
                        ...
                        <?php foreach ($rows as $row): ?>
                            <div class="mobile-card">
                                <div class="d-flex justify-content-between gap-2">
                                    <div>
                                        <div class="mobile-card-title"><?= e((string)($row['job_no'] ?? 'Job Cards')) ?></div>
                                        <span class="mobile-card-subtitle">Job No: <?= e((string)($row['job_no'] ?? '-')) ?></span>
...
<span class="mobile-card-subtitle">Delivery Date: <?= e((string)($row['delivery_date'] ?? '-')) ?></span>
                                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <form method="post" class="modal-content">
                ...
                <div class="modal-header">
                    <h5 class="modal-title fw-bold" id="recordModalTitle">Create Job Cards</h5>
                    <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                </div>

                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-6"><label class="form-label fw-bold">Job No</label><input type="text" name="job_no" id="job_no" class="form-control"></div>
...
<div class="col-md-6"><label class="form-label fw-bold">Delivery Date</label><input type="date" name="delivery_date" id="delivery_date" class="form-control"></div>
<div class="col-md-6"><label class="form-label fw-bold">Job Stage</label><input type="text" name="job_stage" id="job_stage" class="form-control"></div>
    ...
    <script>
        (function () {
            ...
            document.getElementById('newRecordBtn')?.addEventListener('click', function () {
                title.textContent = 'Create Job Cards';
            ...
            document.querySelectorAll('.js-edit-record').forEach(function (btn) {
                btn.addEventListener('click', function () {
                    title.textContent = 'Edit Job Cards';
                    ...
                    set('job_stage', btn.dataset.jobStage || '');
