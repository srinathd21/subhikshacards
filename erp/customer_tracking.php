...
$icon = $isDone ? '✓' : ($index + 1);
$isOpen = $isLive || $index === $openStepIndex;
?>

            <article class="step <?= $isOpen ? 'open active' : '' ?> <?= $class ?>">

                <button type="button" class="step-button" onclick="toggleStep(this)">

                    <?php if ($isDone): ?>
                    <span class="step-icon">✓</span>
                    <?php else: ?>
                    <span class="step-number <?= $class ?>"><?= $icon ?></span>
                    <?php endif; ?>

                    <span class="step-title"><?= e($step['step_name']) ?></span>

                    <span class="step-status <?= $class ?>">
                ...
                <div class="step-details">
                    ...
                    <div class="detail-grid">
                        ...
                        <div class="detail-box">
                            ...
                            <strong>
                                <?= ($isLive || $isDone) ? ctDateTime($step['actual_start_at']) : '-' ?>
                            </strong>
                        </div>
                        ...
                        <div class="detail-box">
                            ...
                            <strong>
                                <?= $isDone ? ctDateTime($step['actual_completed_at']) : '-' ?>
                            </strong>
                        </div>
        ...
        uire_once __DIR__ . '/includes/db.php';
        ...
        if ($balance <= 0 && $final> 0) {
            ...
            : 'NULL AS tracking_expires_at, NULL AS tracking_is_active';
            $trackingJoin = $hasTrackingLinks
            ? 'LEFT JOIN customer_tracking_links ctl ON ctl.job_card_id = jc.id' . ($token !== '' ? ' AND
            ctl.tracking_token = ?' : '')
            : '';
            ...
            $message = $token !== '' ? 'Tracking link not found.' : 'No order found for this job card number.';
            $messageType = 'danger';
            } elseif ($token !== '' && isset($job['tracking_is_active']) && $job['tracking_is_active'] !== null &&
            (int)$job['tracking_is_active'] !== 1) {
            $message = 'This tracking link is inactive.';
            $messageType = 'danger';
            $job = null;
            } elseif ($token !== '' && !empty($job['tracking_expires_at']) &&
            strtotime((string)$job['tracking_expires_at']) < time()) { $message='This tracking link has expired.' ;
                $messageType='danger' ; $job=null; } } catch (Throwable $e) {
                $message='Unable to load tracking details.' ; $messageType='danger' ; $job=null; } } if ($job) { try {
                $stmt=$conn->prepare("
                SELECT
                jt.*,
                ws.step_name,
                        ...
                        <form method="get" class="search-card card" autocomplete="off">
                            ...
                            <div class="search-row">
                                <input type="text" id="job_card_no" name="job_card_no"
                                    ...
                                    placeholder="Example: JC-000125" required>
                                <button type="submit">Track</button>
                            </div>
                        </form>

                        <?php if ($message !== ''): ?>
                        <div class="message <?= e($messageType === 'danger' ? 'danger' : '') ?>"><?= e($message) ?>
                        </div>
                        <?php endif; ?>
            ...
            ?>
                        ...
                        ?>
                            <article
                                class="step <?= e($isActive ? 'active' : '') ?> <?= e($class) ?> <?= e($isOpen ? 'open' : '') ?>">
                        ...
                        <p class="footer-note">This is a live customer tracking page from Subhiksha Cards. Please
                            contact the team for urgent changes.</p>
                        <?php endif; ?>
                    </main>
