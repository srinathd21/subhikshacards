...
function st_redirect($q=''){header('Location: system-config.php'.($q?'?'.$q:''));exit;}
...
?>
<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>System Configuration - Subhiksha Cards</title>
...
<section class="page-section master-page"><div class="card-ui page-head"><div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3"><div><h1 class="mb-1">System Configuration</h1><p class="text-muted-custom mb-0">Quick configuration view for application level settings.</p></div><button id="newSettingBtn" class="btn btn-primary rounded-pill px-4 fw-bold" data-bs-toggle="modal" data-bs-target="#settingModal">New Setting</button></div></div>
