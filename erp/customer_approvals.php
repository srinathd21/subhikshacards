...
<head>
    ...
    .approval-card-danger {
        ...
        background: color-mix(in srgb, #fee2e2 38%, var(--card-bg)) !important
    }
                ...
                <div class="card-ui page-head">
                    <div class="d-flex flex-column flex-lg-row align-items-lg-center justify-content-between gap-3">
                        <div>
                            ...
                            <p class="text-muted-custom mb-0">Waiting customer approvals with related job card details.
                            </p>
                        </div>
                        <a href="job_cards.php" class="btn btn-outline-secondary rounded-pill px-4 fw-bold">Back to Job
                            Cards</a>
                    </div>
                </div>
                ...
                <div class="row g-3 mb-3">
                    <div class="col-6 col-md-3">
                        <div class="card-ui stat-card h-100">
                            ...
                            <div><span>Waiting</span><strong><?= number_format($stats['waiting']) ?></strong></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card-ui stat-card h-100">
                            ...
                            <div><span>Approved</span><strong><?= number_format($stats['approved']) ?></strong></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card-ui stat-card h-100">
                            <div class="stat-icon" style="background:linear-gradient(135deg,#dc2626,#f43f5e)"><i
                                    data-lucide="x-circle"></i></div>
                            <div><span>Rejected</span><strong><?= number_format($stats['rejected']) ?></strong></div>
                        </div>
                    </div>
                    <div class="col-6 col-md-3">
                        <div class="card-ui stat-card h-100">
                            <div class="stat-icon" style="background:linear-gradient(135deg,#2563eb,#0ea5e9)"><i
                                    data-lucide="list-checks"></i></div>
                            <div><span>Total</span><strong><?= number_format($stats['all']) ?></strong></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-ui module-card">
                    <div
                        ...
                        <input type="search" id="tableSearch" class="form-control" style="max-width:340px"
                            placeholder="Search approval / job card...">
                    </div>

                    <div class="filter-tabs mb-3">
                        <a class="filter-tab <?= $filter === 'waiting' ? 'active' : '' ?>"
                            href="customer_approvals.php?status=waiting">Waiting
                            (<?= number_format($stats['waiting']) ?>)</a>
                        <a class="filter-tab success <?= $filter === 'approved' ? 'active' : '' ?>"
                            href="customer_approvals.php?status=approved">Approved
                            (<?= number_format($stats['approved']) ?>)</a>
                        <a class="filter-tab danger <?= $filter === 'rejected' ? 'active' : '' ?>"
                            href="customer_approvals.php?status=rejected">Rejected
                            (<?= number_format($stats['rejected']) ?>)</a>
                        <a class="filter-tab <?= $filter === 'expired' ? 'active' : '' ?>"
                            href="customer_approvals.php?status=expired">Expired
                            (<?= number_format($stats['expired']) ?>)</a>
                        <a class="filter-tab <?= $filter === 'all' ? 'active' : '' ?>"
                            href="customer_approvals.php?status=all">All (<?= number_format($stats['all']) ?>)</a>
                    </div>
                        ...
                        <table class="table-ui" id="dataTable">
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td><strong><?= e($row['step_name'] ?? '-') ?></strong><small
                                            class="d-block text-muted-custom"><?= e(caApprovalLabel($approvalType)) ?></small>
                                    </td>
                                    <td><span
                                            class="amount-text"><?= e(caMoney($row['final_amount'] ?? 0)) ?></span><small
                                            class="d-block text-muted-custom">Bal:
                                            <?= e(caMoney($row['balance_amount'] ?? 0)) ?></small></td>
                                    <td><?= e(caDate($row['delivery_date'] ?? null)) ?></td>
                                    <td><span
                                            class="status-pill <?= e($statusClass) ?>"><?= e($bucket === 'waiting' ? 'Waiting' : ucwords(str_replace('_', ' ', $status))) ?></span>
                                    </td>
                                    <td class="text-end">
                                        <button type="button"
                                            ...
                                            data-display-status="<?= e($bucket === 'waiting' ? 'Waiting' : ucwords(str_replace('_', ' ', $status))) ?>"><i
                                                data-lucide="eye"></i></button>
                                        <a href="job_card_view.php?id=<?= e($row['job_card_id'] ?? 0) ?>"
                                            class="btn btn-sm btn-outline-primary rounded-circle fw-bold btn-action-icon"
                                            title="Open Job Card"><i data-lucide="briefcase-business"></i></a>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                            </tbody>
                        ...
                        <div class="mobile-card">
                            <div class="d-flex justify-content-between gap-2">
                                <div>
                                    ...
                                    <span class="mobile-card-subtitle">Approval:
                                        <?= e(caApprovalLabel($approvalType)) ?></span>
                                    <span class="mobile-card-subtitle">Delivery:
                                        <?= e(caDate($row['delivery_date'] ?? null)) ?></span>
                                </div>
                                <span
                                    class="status-pill <?= e($statusClass) ?>"><?= e($bucket === 'waiting' ? 'Waiting' : ucwords(str_replace('_', ' ', $status))) ?></span>
                            </div>
                            <div class="mobile-card-actions">
                                <button type="button"
                                    ...
                                    data-bs-toggle="modal" data-bs-target="#viewModal"
                                    <?php foreach ($row as $key => $value): ?>
                                    data-<?= e(str_replace('_', '-', $key)) ?>="<?= e($value) ?>" <?php endforeach; ?>
                                    data-approval-label="<?= e(caApprovalLabel($approvalType)) ?>"
                                    data-display-status="<?= e($bucket === 'waiting' ? 'Waiting' : ucwords(str_replace('_', ' ', $status))) ?>"><i
                                        data-lucide="eye"></i></button>
                                <a href="job_card_view.php?id=<?= e($row['job_card_id'] ?? 0) ?>"
                                    class="btn btn-sm btn-outline-primary rounded-circle fw-bold btn-action-icon"><i
                                        data-lucide="briefcase-business"></i></a>
                            </div>
                        </div>
                        <?php endforeach; ?>
                    </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-xl">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="row g-3">
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Job Card</small><strong id="m_job_card_no">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Job Status</small><strong
                                    id="m_job_status_name">-</strong></div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Customer</small><strong id="m_customer_name">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Mobile</small><strong id="m_mobile">-</strong></div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Product</small><strong id="m_product_name">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Printing</small><strong id="m_printing_name">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Delivery Date</small><strong
                                    id="m_delivery_date">-</strong></div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Balance</small><strong id="m_balance_amount">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Stage</small><strong id="m_step_name">-</strong></div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Approval Type</small><strong
                                    id="m_approval_label">-</strong></div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Status</small><strong id="m_display_status">-</strong>
                            </div>
                        </div>
                        <div class="col-md-3">
                            <div class="view-info-card"><small>Approval ID</small><strong id="m_approval_id">-</strong>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Approved By Customer</small><strong
                                    id="m_approved_by_customer">-</strong></div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Approved By Call</small><strong
                                    id="m_approved_by_call">-</strong></div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Call Confirmed By</small><strong
                                    id="m_call_confirmed_by_name">-</strong></div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Approved At</small><strong id="m_approved_at">-</strong>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Rejected At</small><strong id="m_rejected_at">-</strong>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Expires At</small><strong id="m_expires_at">-</strong>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="view-info-card"><small>Approval Token</small><span
                                    id="m_approval_token">-</span></div>
                        </div>
                        <div class="col-md-6">
                            <div class="view-info-card"><small>Customer Remarks</small><span
                                    id="m_customer_remarks">-</span></div>
                        </div>
                        <div class="col-md-6">
                            <div class="view-info-card"><small>Internal Remarks</small><span
                                    id="m_internal_remarks">-</span></div>
                        </div>
                        <div class="col-md-6">
                            <div class="view-info-card"><small>Tracking Remarks</small><span
                                    id="m_tracking_remarks">-</span></div>
                        </div>
                        <div class="col-md-6">
                            <div class="view-info-card"><small>Delay Remarks</small><span id="m_delay_remarks">-</span>
                            </div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Planned Completion</small><strong
                                    id="m_planned_completion_date">-</strong></div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>Actual Completed</small><strong
                                    id="m_actual_completed_at">-</strong></div>
                        </div>
                        <div class="col-md-4">
                            <div class="view-info-card"><small>IP Address</small><strong id="m_ip_address">-</strong>
                            </div>
                        </div>
                        <div class="col-12">
                            <div class="view-info-card"><small>User Agent</small><span id="m_user_agent">-</span></div>
                        </div>
                    </div>
                </div>
    ...
    (function() {
        ...
        document.querySelectorAll('.js-view-record').forEach(function(btn) {
            btn.addEventListener('click', function() {
                ...
                setText('m_printing_name', [btn.dataset.printingName || '-', btn.dataset
                    .subTypeName || ''
                ].join(' ').trim());
                ...
                setText('m_approved_by_customer', Number(btn.dataset.approvedByCustomer || 0) ===
                    1 ? 'Yes' : 'No');
                setText('m_approved_by_call', Number(btn.dataset.approvedByCall || 0) === 1 ?
                    'Yes' : 'No');
                ...
                if (link) link.href = 'job_card_view.php?id=' + encodeURIComponent(btn.dataset
                    .jobCardId || '0');
            });
        });
